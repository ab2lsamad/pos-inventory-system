...
import { manualAdjustSchema, type ManualAdjustFormData } from '@/schemas/inventory';
import { useManualAdjust } from '@/hooks/inventory/useManualAdjust';
...
const defaults: ManualAdjustFormData = {
  storeId: '',
  variantId: '',
  deltaStr: '',
  reason: '',
...
export default function ManualAdjustModal({
  ...
  const adjust = useManualAdjust();
  const [selectedVariantItem, setSelectedVariantItem] =
    useState<SearchableSelectItem | null>(null);

  const form = useForm<ManualAdjustFormData>({
    resolver: zodResolver(manualAdjustSchema),
    defaultValues: defaults,
  });

  useEffect(() => {
    if (!isOpen) return;
    form.reset({ storeId: defaultStoreId ?? '', variantId: '', deltaStr: '', reason: '' });
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      await adjust.mutateAsync({
        ...
        delta: parseInt(values.deltaStr, 10),
  ...
  return (
    <Modal
      ...
      formId="manual-adjust-form"
      onCancel={onClose}
      isLoading={adjust.isPending}
      confirmLabel="Apply Adjustment"
    >
      <form id="manual-adjust-form" onSubmit={onSubmit} className="space-y-4">
        <Controller
        ...
        <FormField<ManualAdjustFormData>
          name="deltaStr"
          control={form.control}
          label="Delta (positive = add, negative = remove)"
        ...
        <FormField<ManualAdjustFormData>
          ...
          control={form.control}
          label="Reason"
          placeholder="Why are you adjusting inventory?"
        />
      </form>
    </Modal>
  );
